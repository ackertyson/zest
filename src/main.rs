...
fn parse_gradient(val: &str) -> Option<GradientPair> {
...
fn parse_flip_rate(val: &str) -> Result<usize, String> {
    let n = val.parse::<usize>().map_err(|_| {
        format!(
            "error: --flip-rate value must be a positive integer, got {:?}",
            val
        )
    })?;
    if !(FLIP_RATE_MIN..=FLIP_RATE_MAX).contains(&n) {
        return Err(format!(
            "error: --flip-rate must be between {} and {}",
            FLIP_RATE_MIN, FLIP_RATE_MAX
        ));
    }
    Ok(n)
}
...
fn parse_duration(val: &str) -> Result<u64, String> {
    let ms = val.parse::<u64>().map_err(|_| {
        format!(
            "error: --duration value must be a positive integer, got {:?}",
            val
        )
    })?;
    if !(DURATION_MIN_MS..=DURATION_MAX_MS).contains(&ms) {
        return Err(format!(
            "error: --duration must be between {} and {} ms",
...
fn parse_cli_args() -> CliArgs {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--flip-rate" => match args.next() {
                None => {
                    eprintln!("error: --flip-rate requires a value");
                    std::process::exit(1);
                }
...
fn main() {
    ...
    if zsh {
        ...
        write!(
            out,
            "{}%{{\x1b[?25h%}}",
            shell::wrap_ansi_for_zsh(&raw_input)
        )
        .unwrap();
    } else {
...
mod tests {
    ...
    fn gradient_fg_only() {
        assert_eq!(
            parse_gradient("226,220,214"),
            Some((Some(vec![226, 220, 214]), None))
        );
    }
    ...
    fn gradient_fg_and_bg() {
        assert_eq!(
            parse_gradient("226,220:52,88"),
            Some((Some(vec![226, 220]), Some(vec![52, 88])))
        );
    }
    ...
    fn gradient_bg_only() {
        assert_eq!(
            parse_gradient(":52,22,18"),
            Some((None, Some(vec![52, 22, 18])))
        );
    }
    ...
    fn gradient_whitespace_trimmed() {
        ...
        assert_eq!(
            parse_gradient("10, 20 : 30, 40"),
            Some((Some(vec![10, 20]), Some(vec![30, 40])))
        );
    }
